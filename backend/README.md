...
```bash
python seed_demo.py                              # 로컬(http://127.0.0.1:8000) 대상
python seed_demo.py https://<배포주소>            # 배포된 서버 대상
```

임대인 1명(`01000000001`/`demo1234!`) + 임차인 1명(`01000000002`/`demo1234!`)을 만들어서 건물·계약·수리업체·사전합의사항·납부내역(완료/연체/D-3 예정)·문제접수(해결완료 1건 + 협의중 1건)·채팅까지 한 번에 채워준다. **여러 번 실행해도 안전**(이미 있는 데이터는 재사용) — 재배포로 DB가 초기화된 뒤에도 그냥 다시 실행하면 됨. 표준 라이브러리만 써서 `pip install` 없이 바로 실행 가능.
